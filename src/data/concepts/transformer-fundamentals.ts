...
Output layer: final hidden state → Linear(vocab) → Softmax → probabilities`,
    ...
    steps: [
      { title: bi('Tokenize', 'Tokenize'), caption: bi('Text → IDs', 'Text → IDs') },
      { title: bi('Embed', 'Embed'), caption: bi('IDs → vectors', 'IDs → vectors') },
      { title: bi('Attention layers', 'Attention layers'), caption: bi('L × transformer block', 'L × transformer block') },
      { title: bi('Linear head', 'Linear head'), caption: bi('Hidden → vocab scores', 'Hidden → vocab scores') },
      { title: bi('Softmax', 'Softmax'), caption: bi('Next token prob', 'Next token prob') },
    ],
    teaching: teaching({
